        ...
        <div class="col-sm-9 page-content">
          ...
          <div class="inner-box">
            <div class="welcome-msg">
              <h3 class="page-sub-header2 clearfix no-padding">Ads </h3>
            ...
            <div>
              <table class="table table-striped table-bordered add-manage-table table demo footable-loaded footable">
                ...
                <tr>
                  <td>{{ $row->title }}</td>
                  <td>{{ $row->price }}</td>
                  <td>{{ $category[$row->subcategory_id] or '' }}</td>
                  <td>{{ $city[$row->city_id] or '' }}</td>
                  <td>{{ $ad_status[$row->status] or '' }}</td>
                  <td>
                    <p><a class="btn btn-info btn-xs" href="{{ route('admin-ad-edit', $row->id) }}"> <i class="fa fa-recycle"></i> Edit </a> </p>
                    
                    <p><a class="btn btn-danger btn-xs" href="{{ route('admin-ad-delete', $row->id) }}"> <i class=" fa fa-trash"></i> Reject </a></p>
                    
                  </td>
                </tr>
                @endforeach
				<tr>
					<td colspan="5">{{ $ads->links() }}</td>
				</tr>
